...
class TripAdapter(
    ...
    private val dateFmt: DateFormat = DateFormat.getDateInstance(DateFormat.MEDIUM, Locale.getDefault())
    private val currencyFmt: NumberFormat = NumberFormat.getCurrencyInstance()
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): VH {
        ...
        return VH(v, dateFmt, currencyFmt)
    }
    ...
    class VH(
        ...
        fun bind(item: Trip, onDelete: (Trip) -> Unit, onEdit: (Trip) -> Unit, onOpen: (Trip) -> Unit) {
            ...
            if (item.budget > 0.0) {
                ...
                budgetPill.text = currencyFmt.format(item.budget)
            } else {
